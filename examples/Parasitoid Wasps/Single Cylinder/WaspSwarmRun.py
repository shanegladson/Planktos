...
adjustingForAngle = True  # Set to True if you want the agents to compensate for fluid flow
...
# Calculate the angle of the wasps stuck on the trap relative to the reference vector
stuckLocs = swrm.get_prop('stick')
calcVectors = (swrm.positions - targets)[stuckLocs == True]
angles = np.arctan2(calcVectors[:,1], calcVectors[:,0])
maxAngle = np.abs(max(angles))
plt.ylim((-maxAngle, maxAngle))

plt.title('Distribution of Landing Spots')
plt.ylabel('Angle (radians)')
plt.xlabel('Wasps Trapped')
...
sns.histplot(y=angles, bins=30, kde=True)
plt.show()
